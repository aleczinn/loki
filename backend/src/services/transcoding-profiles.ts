...
export function getTranscodingArgs(session: PlaySession): TranscodingArgs {
    ...
    // SUBTITLE BURN-IN
    // Muss VOR den Video-Args eingefügt werden, weil es den -vf Filter erweitert!
    if (session.subtitleIndex >= 0 && decision.subtitle.action === 'burn_in') {
        const subtitleTrack = file.metadata.subtitle[session.subtitleIndex];

        if (subtitleTrack) {
            const format = subtitleTrack.Format?.toLowerCase() || '';
            const isTextBased = ['subrip', 'utf-8', 'ass', 'ssa', 'webvtt'].includes(format);
            const isBitmap = ['pgs', 'vobsub', 'hdmv_pgs_subtitle', 'dvd_subtitle'].includes(format);

            if (isTextBased) {
                // Text-basierte Subs: über subtitles Filter
                // StreamOrder ist der absolute Index im Container
                const streamOrder = parseInt(subtitleTrack.StreamOrder || '0');

                // Escape den Dateipfad für FFmpeg Filter (Backslash, Doppelpunkt etc.)
                const escapedPath = file.path
                    .replace(/\\/g, '/')
                    .replace(/:/g, '\\:')
                    .replace(/'/g, "\\'");

                // Prüfe ob bereits ein -vf existiert und erweitere ihn
                const existingVfIndex = videoArgs.indexOf('-vf');
                if (existingVfIndex !== -1) {
                    // Hänge Subtitle-Filter an bestehenden -vf an
                    videoArgs[existingVfIndex + 1] += `,subtitles='${escapedPath}':si=${session.subtitleIndex}`;
                } else {
                    videoArgs.push('-vf', `subtitles='${escapedPath}':si=${session.subtitleIndex}`);
                }
            } else if (isBitmap) {
                // Bitmap-Subs (PGS, VobSub): über overlay Filter
                // Erfordert ein separates Input-Mapping
                const streamOrder = parseInt(subtitleTrack.StreamOrder || '0');

                const existingVfIndex = videoArgs.indexOf('-vf');
                if (existingVfIndex !== -1) {
                    videoArgs[existingVfIndex + 1] += `,[0:s:${session.subtitleIndex}]overlay`;
                } else {
                    videoArgs.push('-filter_complex',
                        `[0:v:0][0:s:${session.subtitleIndex}]overlay`
                    );
                }
            }
        }
